...
for file_path in path_files:
    ...
    for algo_name, df in blocks.items():
        ...
        # Add best path
        best_path_list = list(df['id'].astype(str))
        best_path_text = " - ".join(best_path_list)
        print("Best Path:", best_path_text)

        # store for latex table:
        safe_algo_name = algo_name.replace(" ", "_")
        latex_rows.append((f"{base_name}_{safe_algo_name}", best_path_text))

        plt.figtext(0.5, 0.1, f"Best Path: {best_path_text}", wrap=True, horizontalalignment='center', fontsize=12)
...
# --- SAVE LATEX TABLE (two-row format, wrapped text) ---
latex_output = []
latex_output.append("\\begin{table}[h!]")
latex_output.append("\\centering")
latex_output.append("\\begin{tabular}{|p{14cm}|}")
latex_output.append("\\hline")

for file_name, best_path in latex_rows:
    escaped_file = file_name.replace("_", "\\_")
    escaped_path = best_path.replace("_", "\\_")

    latex_output.append(f"\\textbf{{{escaped_file}}} \\\\ \\hline")
    latex_output.append(f"{escaped_path} \\\\ \\hline")

latex_output.append("\\end{tabular}")
latex_output.append("\\caption{Best paths for each algorithm}")
latex_output.append("\\end{table}")

with open("best_paths.tex", "w") as f:
    f.write("\n".join(latex_output))

print("Saved LaTeX table to best_paths.tex")
