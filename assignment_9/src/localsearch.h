...
vector<int> steepest_local_search_new(
    ...
    const int path_size = current_path.size();

    // Prepare the list of nodes not currently in the solution
    ...
    for (const auto& node : nodes) {
        if (selected_nodes.find(node.id) == selected_nodes.end()) {
            not_in_solution.push_back(node.id);
        }
    }

    if (path_size < 2) return current_path;

    while (true) {
        double best_delta = 0.0;
        int best_move_type = -1; // 0 = intra-node, 1 = intra-edge, 2 = inter-node
        int best_first_node = -1;
        ...
        for (int i = 0; i < path_size; ++i) {
            for (int node_id : not_in_solution) {
                double delta = delta_inter_node(current_path, i, node_id, dist_matrix, nodes);
                if (delta < best_delta) {
                    best_delta = delta;
                    best_move_type = 2;
                    best_first_node = i;
                    best_not_selected_node = node_id;
                }
            }
        }
        ...
        if (best_move_type == 0) {
            ...
            // Intra-edge 2-opt swap (reverse segment)
            int start = (best_first_node + 1) % path_size;
            int end = best_second_node;
            int num_to_swap = (start <= end) ? (end - start + 1) : (path_size - start + end + 1);
            ...
            for (int k = 0; k < num_to_swap / 2; ++k) {
                ...
                i = (i + 1) % path_size;
                j = (j - 1 + path_size) % path_size;
            }
        } else if (best_move_type == 2) {
            // Inter-node swap
            int old_node = current_path[best_first_node];
            current_path[best_first_node] = best_not_selected_node;

            // Update not_in_solution list
            auto it = find(not_in_solution.begin(), not_in_solution.end(), best_not_selected_node);
